...
contract Ecto is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 ECTO-WETH LP -> ECTO pool
    uint256 public constant INITIAL_ECTO_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the ECTO ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Ecto TEST Token", "TECTO") {
        // Mints 1 ECTO to contract creator for initial pool setup
    ...
    function _getEctoPrice() internal view returns (uint256 _ectoPrice) {
        try IOracle(ectoOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Ecto: failed to fetch ECTO price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ectoPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_ectoPrice >= taxTiersTwaps[tierId]) {
    ...
    function setEctoOracle(address _ectoOracle) public onlyOperatorOrTaxOffice {
        require(_ectoOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints ECTO to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ECTO to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentEctoPrice = _getEctoPrice();
            currentTaxRate = _updateTaxRate(currentEctoPrice);
            if (currentEctoPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_ectoPool != address(0), "!_ectoPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_ectoPool, INITIAL_ECTO_POOL_DISTRIBUTION);
    }
