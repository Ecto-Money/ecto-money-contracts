...
contract EShareRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. eSHARE to distribute per block.
        uint256 lastRewardTime; // Last time that eSHARE distribution occurs.
        uint256 accBSharePerShare; // Accumulated eSHARE per share, times 1e18. See below.
    ...
    // The time when eSHARE mining starts.
    uint256 public poolStartTime;

    // The time when eSHARE mining ends.
    uint256 public poolEndTime;

    uint256 public eSharePerSecond = 0.00835905 ether; // 65000 bshare / (90 days * 24h * 60min * 60s)
    uint256 public runningTime = 90 days; // 370 days
    ...
    // Update the given pool's eSHARE allocation point. Can only be called by the owner.
    ...
    function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime) return poolEndTime.sub(poolStartTime).mul(eSharePerSecond);
            return poolEndTime.sub(_fromTime).mul(eSharePerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime) return _toTime.sub(poolStartTime).mul(eSharePerSecond);
            return _toTime.sub(_fromTime).mul(eSharePerSecond);
        }
    }

    // View function to see pending eSHARE on frontend.
    ...
    // Safe bshare transfer function, just in case if rounding error causes pool to not have enough eSHARE.
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (eSHARE or lps) if less than 90 days after pool ends
